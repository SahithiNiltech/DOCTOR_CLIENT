...
def client_registerApi():
    ...
    height=data["height"]
    weight=data["weight"]
    res_bmi=float(weight)/(float(height)**2)
    bmi=round(res_bmi,1)
    # print(bmi)
    db = sqlite3.connect("doctor_clients.db")
    col = db.execute(f'''SELECT * FROM patients WHERE Mobile_number = '{mob_numb}';''').fetchall()
    if len(col)==0:
        col=db.execute(f'''INSERT INTO patients (First_Name,Last_Name,Email, Password,Mobile_number,Height,Weight,BMI)
                        VALUES ('{fname}','{lname}','{email_id}', '{password}','{mob_numb}','{height}','{weight}','{bmi}');''')
        db.commit();
        db.close()
...
def localities():
    if "phone" not in session:
        return redirect("/")
    db = sqlite3.connect("doctor_clients.db")
    data = db.execute('''SELECT DISTINCT Locality FROM doctor_table''').fetchall()
    db.close()    
    return render_template("localities.html",data=data)
...
def doctor_list(location):
    if "phone" not in session:
        return redirect("/")
    db = sqlite3.connect("doctor_clients.db")
    data = db.execute(f'''SELECT Name,Mobile_number,Locality,Specialization FROM doctor_table 
                      WHERE Locality = '{location}';''').fetchall()
    db.close()
    # print(data)
...
def appointment_confirm(doctor_number):
    ...
    db = sqlite3.connect("doctor_clients.db")
    pat_data=db.execute(f'''SELECT First_Name,BMI FROM patients WHERE Mobile_number={session["phone"]} ''').fetchone()
    patient_name=pat_data[0]
    patient_bmi=pat_data[1]
    # print(pat_data)
    db.close()
    ...
    if len(data)==0:
        db.execute(f'''INSERT INTO doctor_appointments (Date,Time,Patient_mob_num,Patient_Name, Doctor_mob_num,Patient_BMI)
                        VALUES ('{date}','{time}','{session["phone"]}', '{patient_name}','{doctor_number}','{patient_bmi}');''')
        db.commit()
        db.close()    
    else:
...
def doctor_upload_file(patient_numb,date):
    ...
    report_data=db.execute(f'''SELECT Filename FROM doctor_reports 
                            WHERE Doctor_mob_num='{session["doctor"]}' 
                            and Patient_mob_num={patient_numb}; ''').fetchall()
    db.close()
